...
class ServiceManagementMain(DigitalPyService):
    ...
    def handle_command(self, command: Response):
        ...
        if command.get_value(COMMAND) == str(
            ServiceManagerOperations.START_SERVICE.value
        ):
            self.start_service(command.get_value(SECTION_NAME))

        elif command.get_value(COMMAND) == str(
            ServiceManagerOperations.STOP_SERVICE.value
        ):
            self.stop_service(command.get_value(TARGET_SERVICE_ID))

        elif command.get_value(COMMAND) == str(
            ServiceManagerOperations.RESTART_SERVICE.value
        ):
            self.stop_service(command.get_value(TARGET_SERVICE_ID))
            self.start_service(command.get_value(SECTION_NAME))

        elif command.get_value(COMMAND) == str(
            ServiceManagerOperations.GET_ALL_SERVICE_HEALTH.value
        ):
    ...
    def send_response_to_core(self, message: Any, command_id: str):
        ...
        from digitalpy.core.main.DigitalPy import (
            DigitalPy,
    ...
    @staticmethod
    def _create_service_description(service_name: str) -> ServiceDescription:
        """
        Create a service description object based on the provided service name.
        This is temporary, provided as a utility for migration purposes.

        Args:
            service_name (str): The name of the service.

        Returns:
            ServiceDescription: The created service description object.
        """
        warnings.warn(
            "This method is deprecated and should only be used for the purposes of migration",
            DeprecationWarning,
        )

        # initialize the configuration
        configuration: Configuration = ObjectFactory.get_instance("configuration")

        # get the service configuration
        service_configuration: dict = configuration.get_section(service_name)

        # construct the service description
        new_service_instance = ServiceDescription()

        # set the id of the service
        new_service_instance.id = service_configuration.get("service_id")
