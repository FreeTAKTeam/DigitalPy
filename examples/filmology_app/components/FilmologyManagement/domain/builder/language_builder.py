...
from filmology_app.components.FilmologyManagement.configuration.FilmologyManagement_constants import LANGUAGE
...
class LanguageBuilder(Builder):
    """Builds a Language object"""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.result: Language = None  # type: ignore

    def build_empty_object(self, config_loader, *args, **kwargs):  # pylint: disable=unused-argument
        """Builds a Language object"""
        self.request.set_value("object_class_name", "Language")

        configuration = config_loader.find_configuration(LANGUAGE)

        self.result = super()._create_model_object(
          configuration, extended_domain={"Language": Language,
                                        })

    def add_object_data(self, mapped_object: Union[bytes, str, DBLanguage], protocol=None):
        ...
        elif isinstance(mapped_object, DBLanguage):
    ...
    def _add_db_object_data(self, db_object: DBLanguage):
        ...
        self.result.shortForm = db_object.shortForm
